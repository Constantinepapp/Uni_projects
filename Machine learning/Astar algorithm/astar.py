...
def pop_up_window(app):
    ...
    Label(win, text="Black tiles are flat terrain (cost = 1)",
          font=("Calibri", 13), pady=5, padx=10).pack()
    Label(win, text="Green tiles are forest area (cost = 2)",
          font=("Calibri", 13), pady=5, padx=10).pack()
    Label(win, text="Blue tiles are water bodies (cost = 3)",
          font=("Calibri", 13), pady=5, padx=10).pack()      
...
class App:
    ...
    def __init__(self, master):
        ...
        self.master = master
        master.wm_title("A* Algorithm by Konstantinos Pappas MPPL 20061 and Eleftheria Pavlidi MPPL 200..")
        self.buttons = []
        self.start = []
        self.goal = []
        self.obstacles = []
        ...
        self.weight = createterrain.terrain()
            ...
            for j in range(25):
                ...
                if self.weight[i][j] == 1:
                    button = Button(master, width=2, height=1,
                                command=partial(self.button_operation, i, j), state="disabled",bg="black",highlightthickness = 0, bd = 0)
                elif self.weight[i][j] == 3:
                    button = Button(master, width=2, height=1,
                                command=partial(self.button_operation, i, j), state="disabled",bg="green",highlightthickness = 0, bd = 0)
                else:
                    button = Button(master, width=2, height=1,
                                command=partial(self.button_operation, i, j), state="disabled",bg="blue",highlightthickness = 0, bd = 0)
    ...
    def button_operation(self, row, column):
        ...
        if self.mode == 0:
            ...
            self.mode = 1
            self.buttons[row][column].configure(bg='white')
    ...
    def add_obstacle(self, row, column, event):
        ...
        if self.mode == 3:
            ...
            self.buttons[row][column].configure(bg='brown')
    ...
    def reconstruct_path(self, cameFrom, current):
        ...
        while current != self.start:
            ...
            self.buttons[current[0]][current[1]].configure(bg='white')
    ...
    def a_star_algorithm(self, start, goal):
        ...
        print("staring f_score",f_score[start[0]][start[1]])
        while len(open_set) > 0:
            ...
            for node in neighbors:
                ...
                #print(self.weight[current_row][current_column])
                tentative_gScore = g_score[current_row][current_column] + self.weight[node_row][node_column]
                #print(tentative_gScore,"----",g_score[node_row][node_column])
                if tentative_gScore < g_score[node_row][node_column]:
                    ...
                    print(node_row,node_column,'\n\n',"loop f_score ",f_score[node_row][node_column],"\n gscore ",g_score[node_row][node_column],"\n heuristic",self.heuristic(node, self.goal))
                    if node not in open_set:
                        print("current",current,"came from",came_from)
